...
class DAConvPack(DAConv):
    ...
    def forward(self, x):
        # 1. 提取形变特征 (5个通道)
        out = self.conv_angel(x)

        # 2. 剥离系数和门控
        coeffs = out[:, :4, :, :, :]  # 取前 4 个通道作为多项式参数 (a, b)
        gate = out[:, 4:, :, :, :]  # 取第 5 个通道作为门控掩码 (Mask)

        # 3. 处理多项式系数 (配合底层修改，稍微放宽曲率限制到 0.5)
        coeffs = self.In(coeffs)
        coeffs = (np.pi/4) * torch.tanh(coeffs)

        # 4. 处理门控权重 (映射到 0~1 之间)
        gate = torch.sigmoid(gate)

        # 5. 【核心】：系数与门控相乘，过滤无关区域的形变
        offset = coeffs * gate

        # 6. 底层调用依然只传入 4 通道的 offset，无缝衔接 C++ 代码
        if self.axis == 0:
            snake_conv = DAConv_Function(x, offset, self.weight, self.bias, (self.stride, 1, 1), (self.padding, 0, 0),
                                         self.dilation,
                                         self.groups, self.deformable_groups, self.in_step, self.axis, self.scale)
        elif self.axis == 1:
            snake_conv = DAConv_Function(x, offset, self.weight, self.bias, (1, self.stride, 1), (0, self.padding, 0),
                                         self.dilation,
                                         self.groups, self.deformable_groups, self.in_step, self.axis, self.scale)
        else:
            snake_conv = DAConv_Function(x, offset, self.weight, self.bias, (1, 1, self.stride), (0, 0, self.padding),
                                         self.dilation,
                                         self.groups, self.deformable_groups, self.in_step, self.axis, self.scale)
